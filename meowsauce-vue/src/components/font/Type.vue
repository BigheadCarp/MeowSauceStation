...
<script>
  export default {
    ...
    mounted() {
      this.loadArticles()
      this.$emit('changeArticleType',this.$route.query.type)
    },
    methods: {
      loadArticles() {
        this.$axios.get('/api/type/' + this.pageSize + '/1/' + this.$route.query.type).then(resp => {
      ...
      handleCurrentChange(page) {
        ...
        if (this.articleType.length > 0) {
          this.$axios.get('/api/type' + this.pageSize + '/' + page + '/' + this.$route.query.type).then(resp => {
